...
from app.utils.image_utils import validate_image, preprocess_image, save_thumbnail
...
def analyze():
    ...
    # ── Save thumbnail from first available image ──────────────────────
    thumbnail_path = None
    thumb_source = face_image if face_file else body_image
    if thumb_source is not None:
        analysis_id_preview = str(__import__("uuid").uuid4())
        thumbnail_path = save_thumbnail(
            thumb_source, current_app.config["THUMBNAIL_FOLDER"], analysis_id_preview
        )
    ...
    # Update thumbnail path with actual analysis id
    if thumbnail_path:
        import os
        ...
        if os.path.exists(thumbnail_path):
            os.rename(thumbnail_path, new_thumb)
        analysis.thumbnail_path = new_thumb
